...
from MAIN.Core import SlasherBKBCore as sbc, BaseFunc as bd, Wifu_core as wfc
...
class SlasherBKB(QWidget):
    ...
    def launch_slash(self):
        ...
        if not self.pre_process_check():
            ...
            try:
                wf_check = wfc.run_wifu(self.input_path.text(), self.path_to_wif_state, self.noise_mode, self.increase_mode,self.increase_strength, self.strength_noise, self.type_increase)
                if wf_check == "ЗАВАЙФЛЕНО":
                    self.input_path.setText(self.input_path.text() + "[WIF]")
                ...
                self.status.setText("Вайфу крашнуло(")

    def pre_process_check(self):
        # Проверяем настройки вайфы
        wifu_dir_ch = bd.get_recent_set()
        self.before_slash_wif_mode = wifu_dir_ch.get("before_slash_wif_mode").lower() == "true"
        self.path_to_wif_state = wifu_dir_ch.get("path_to_wif_state")
        self.increase_mode = wifu_dir_ch.get("increase_mode").lower() == "true"
        self.noise_mode = wifu_dir_ch.get("noise_mode").lower() == "true"
        self.increase_strength = wifu_dir_ch.get("increase_strength")
        self.strength_noise = wifu_dir_ch.get("strength_noise")
        self.type_increase = wifu_dir_ch.get("type_increase")
        if self.before_slash_wif_mode:
            if (str(self.path_to_wif_state) == ""):
                self.status.setText("Перепроверьте настройки вайфу!")
                return False
    ...
    def unload_resent(self):
        ...
        if unload_var.get("batch_mode") is not None:
            ...
            self.pixel_checkbox.setChecked(self.pixels_mode)
            self.split_checkbox.setChecked(self.split_mode)
            if hasattr(self, 'scan_line_step_la') and hasattr(self, 'ignorable_edges_pixels_la') and hasattr(self,
                                                                                                             'slicing_sensitivity_la'):
