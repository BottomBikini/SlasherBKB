    ...
    if batch_mode_enabled == False:
        ...
        for fileName in os.listdir(given_input_folder):
            ...
            if os.path.isdir(filePath):
                folder_paths.append((filePath, os.path.join(given_output_folder, fileName + " [Разрезано]")))
    return folder_paths
...
def resize_images(images, width_enforce_type, custom_width=720):
    """Изменяет размер изображений в соответствии с выбранным вами режимом принудительного исполнения."""
...
def new_count_heights(images, count):
...
def split_image(combined_img, split_height, senstivity, ignorable_pixels, scan_step):
    """Разбивает полученное комбинированное изображение на изображения по желаемой высоте."""
    ...
    # Формируем последнее изображение
...
def save_data(data, foldername, outputformat, progress_func=None):
    """Сохраняет заданные изображения в выходной папке."""
